...
Supabase CLI for local development, migrations, and edge functions. Use when the project needs this capability or the user / team manifest asks for it.

## When to use

- `TEAM.yaml` includes `supabase-cli` or user asks by name
...
2. **Confirm tooling** - Verify `supabase-cli` runtime/CLI/MCP is available; document install if missing.
